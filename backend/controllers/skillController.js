...
const addSkill = async (req, res) => {
    try {
        ...
        if (!req.body.iconKey) {
            // Only require file upload when no icon key is specified
            if (!req.file) {
                return res.status(400).json({
                    success: false,
                    message: "Either an icon key or image file is required"
                });
            }
            try {
                const fileInput = req.file.buffer || req.file.path;
                const imageResult = await uploadImageToCloudinary(fileInput, 'skill-icons');
            ...
            // Using icon library key — clear imageUrl
            req.body.imageUrl = '';
...
const updateSkill = async (req, res) => {
    try {
        ...
        if (req.body.iconKey) {
            // Switching to icon library — delete any existing cloudinary image
