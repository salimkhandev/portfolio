...
import {
  ...
  faAndroid, faAws, faGithub, faLinux, faSwift
...
// Available icon options for skill picker
const ICON_OPTIONS = [
  { key: 'faReact',    icon: faReact,    label: 'React',       color: '#61DAFB' },
  { key: 'faAngular',  icon: faAngular,  label: 'Angular',     color: '#DD0031' },
  { key: 'faVuejs',    icon: faVuejs,    label: 'Vue.js',      color: '#4FC08D' },
  { key: 'faNodeJs',   icon: faNodeJs,   label: 'Node.js',     color: '#339933' },
  { key: 'faDocker',   icon: faDocker,   label: 'Docker',      color: '#2496ED' },
  { key: 'faJs',       icon: faJs,       label: 'TypeScript',  color: '#3178C6' },
  { key: 'faHtml5',    icon: faHtml5,    label: 'HTML5',       color: '#E34F26' },
  { key: 'faCss3Alt',  icon: faCss3Alt,  label: 'CSS3',        color: '#1572B6' },
  { key: 'faPython',   icon: faPython,   label: 'Python',      color: '#3776AB' },
  { key: 'faGit',      icon: faGit,      label: 'Git',         color: '#F05032' },
  { key: 'faSass',     icon: faSass,     label: 'Sass',        color: '#CC6699' },
  { key: 'faPhp',      icon: faPhp,      label: 'PHP',         color: '#777BB4' },
  { key: 'faAndroid',  icon: faAndroid,  label: 'Android',     color: '#3DDC84' },
  { key: 'faAws',      icon: faAws,      label: 'AWS',         color: '#FF9900' },
  { key: 'faGithub',   icon: faGithub,   label: 'GitHub',      color: '#181717' },
  { key: 'faLinux',    icon: faLinux,    label: 'Linux',       color: '#FCC624' },
  { key: 'faSwift',    icon: faSwift,    label: 'Swift',       color: '#FA7343' },
  { key: 'faDatabase', icon: faDatabase, label: 'MongoDB',     color: '#47A248' },
  { key: 'faServer',   icon: faServer,   label: 'PostgreSQL',  color: '#336791' },
  { key: 'faBolt',     icon: faBolt,     label: 'Redis',       color: '#DC382D' },
  { key: 'faMobileAlt',icon: faMobileAlt,label: 'PWA',         color: '#5A0FC8' },
  { key: 'faCode',     icon: faCode,     label: 'Next.js',     color: '#000000' },
  { key: 'faShieldAlt',icon: faShieldAlt,label: 'Security',    color: '#f59e0b' },
];

const ICON_MAP_DASH = Object.fromEntries(ICON_OPTIONS.map(o => [o.key, o]));
...
const SkillsView = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || formData.name.trim() === '') {
      swal.fire({ title: 'Skill name is required', icon: 'error' });
      return;
    }
    if (!formData.topics || formData.topics.length === 0) {
      swal.fire({ title: 'At least one topic is required', icon: 'error' });
      return;
    }
    if (!formData.iconKey) {
      swal.fire({ title: 'Please select an icon', icon: 'error' });
      return;
    }
    setUploading(true);
    try {
      ...
      if (isEditing) {
        await axios.put(`${API_BASE_URL}/skills/${isEditing}`, payload);
        swal.fire({ title: 'Success!', text: 'Skill updated successfully', icon: 'success', timer: 2000 });
      } else {
        await axios.post(`${API_BASE_URL}/skills`, payload);
        swal.fire({ title: 'Success!', text: 'Skill added successfully', icon: 'success', timer: 2000 });
      }
      refreshSkills();
      cancelEdit();
    } catch (error) {
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                {ICON_OPTIONS.map(opt => (
                  <button
                    key={opt.key}
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, iconKey: opt.key }))}
                    ...
                    title={opt.label}
                  >
                    <FontAwesomeIcon icon={opt.icon} style={{ color: opt.color, fontSize: '1.5rem' }} />
                    <span className="text-[10px] text-gray-300 truncate w-full text-center">{opt.label}</span>
                  </button>
                ))}
              </div>
              {formData.iconKey && (
                <p className="mt-2 text-xs text-green-400">
                  Selected: {ICON_MAP_DASH[formData.iconKey]?.label}
                </p>
              )}
            </div>
          ...
          {skills.map((skill) => (
            ...
            >
              <div className="flex items-center gap-4 mb-4">
                {/* Show FA icon if iconKey set, else fallback to imageUrl */}
                {skill.iconKey && ICON_MAP_DASH[skill.iconKey] ? (
                  <div className="w-16 h-16 flex items-center justify-center">
                    <FontAwesomeIcon
                      icon={ICON_MAP_DASH[skill.iconKey].icon}
                      style={{ color: ICON_MAP_DASH[skill.iconKey].color, fontSize: '2.5rem' }}
                    />
                  </div>
                ) : skill.imageUrl ? (
                  <img
                  ...
                  <div className="w-16 h-16 flex items-center justify-center">
                    <FontAwesomeIcon icon={faCode} style={{ color: '#6b7280', fontSize: '2.5rem' }} />
                  </div>
                )}
                <div>
