import { HiClock, HiCode, HiExternalLink, HiLightBulb, HiPlay } from "react-icons/hi";
  ...
  return (
    ...
    >
            ...
            <div className="relative w-full" style={{ aspectRatio: "16/9" }}>
              {cloudinaryThumbnailUrl ? (
                ...
                <div className="w-full h-full bg-gray-800/50 flex items-center justify-center">
                  <HiPlay className="w-16 h-16 text-white/50" />
                </div>
              )}
              {/* Play button overlay */}
              <div className="absolute inset-0 flex items-center justify-center bg-black/20 hover:bg-black/30 transition-colors cursor-pointer">
                <HiPlay className="w-16 h-16 text-white/80 hover:text-white transition-all hover:scale-110" />
              </div>
            </div>
          ) : (
            <video
      ...
      <div className="relative p-2 sm:p-6 space-y-2 sm:space-y-4">
        ...
        {/* Duration */}
        {duration && (
          <div className="flex items-center gap-2 pt-1 text-[10px] sm:text-xs text-gray-600">
            <HiClock className="w-3 h-3" style={{ color: '#1f2937' }} />
        ...
        {challenges && (
          <div className="pt-2">
            <div className="flex items-start gap-2">
              <HiLightBulb className="w-3 h-3 mt-0.5" style={{ color: '#1f2937' }} />
              <div>
                <span className="text-[10px] sm:text-xs font-semibold text-gray-800 block mb-1">Challenges:</span>
                <p className="text-[10px] sm:text-xs text-gray-600 leading-relaxed">{challenges}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-2 sm:pt-3">
          ...
          {link && (
            <a
              ...
              className="inline-flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-black hover:bg-gray-800 text-white transition-all duration-300 group/link"
            >
          ...
          {githubLink && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-black border border-gray-200 transition-all duration-300 group/github"
            >
              <HiCode className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
